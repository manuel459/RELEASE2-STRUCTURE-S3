...
try:
    ...
    l_configuraciones = ["REASEGUROS","GENERAL"]
    ...
    for config in l_dic_config['REASEGUROS']['path_file_tmp']:
        if config['flag'] == 1:
            ...
            L_DF_REASEGUROS = structure.get_data(glueContext, connection, l_dic_config['GENERAL']['fechas']['dFecha_Inicio'] , l_dic_config['GENERAL']['fechas']['dFecha_Fin'] )
        
            #Trasformar a bit escrito en formato txt
            L_BUFFER_PRODUCTOS = io.BytesIO()
            L_DF_REASEGUROS.toPandas().to_parquet(L_BUFFER_PRODUCTOS, index=False)
