  ...
  l_abcobap_insunix_lpg = f'''
                        (
                          ...
                          FROM(
                               ...
                               AND GC.EFFECDATE <= (CASE WHEN C.POL_POLITYPE = '1' 
                                                    THEN C.POL_EFFECDATE 
                                                    ELSE C.CERT_EFFECDATE 
                                                    END)
                               AND (GC.NULLDATE IS NULL OR GC.NULLDATE > (CASE WHEN C.POL_POLITYPE = '1' 
                                                    THEN C.POL_EFFECDATE 
                                                    ELSE C.CERT_EFFECDATE 
                                                    END)) LIMIT 1
                               ) ,'0') AS KGCTPCBT,
                               ...
                               COALESCE((COALESCE ((SELECT (SUM(R.SHARE/100)) * C.PREMIUM  
                                                    ...
                                                    AND R.TYPE <> 1),(SELECT (SUM(R.SHARE/100)) * C.PREMIUM  
                                                                      FROM USINSUG01.REINSURAN R 
                                                                      WHERE R.USERCOMP = C.USERCOMP 
                                                                      AND R.COMPANY = C.COMPANY 
                                                                      AND R.CERTYPE = C.CERTYPE  
                            ...
                            FROM(   SELECT 
                                    ...
                                    COALESCE(COALESCE(   (SELECT 	COALESCE(GCO.COVERGEN, 0) || '-' || COALESCE(GCO.CURRENCY, 0) || '-G' 
                                                          ...
                                                          AND (( SELECT DISTINCT PRO.BRANCHT
                                                                 ...
                                                                 AND PRO.NULLDATE IS NULL)) NOT IN ('1', '5') LIMIT 1), ( SELECT 	COALESCE(LCO.COVERGEN, 0) || '-' || COALESCE(LCO.CURRENCY, 0) || '-L' 
                                    ...
                                    COALESCE((SELECT COV.PREMIUM 
                                              ...
                                              AND (COV.NULLDATE IS NULL OR COV.NULLDATE > CERT.DATE_ORIGI) LIMIT 1),(SELECT COV.PREMIUM 
                                                                                                                     FROM USINSUV01.COVER COV 
                                                                                                                     LEFT JOIN USINSUV01.CERTIFICAT CERT                           
                                                                                                                     ON  COV.USERCOMP = CERT.USERCOMP 
                                                                                                                     AND COV.COMPANY  = CERT.COMPANY  
