def get_data(glue_context, connection):
    ...
    L_RBRECPR_INSUNIX_LPG_OTROS_RAMOS = '''
                                           (
                                                        SELECT 	        'D' as INDDETREC,
                                                                        ...
                                                                        '' DCASHBACK --EXCLUIDO 
                                                        from	(select	case	
                                                                                                when dpr.type_detai in ('1','3','4') and dpr.bill_item not in (4,5,9,97)
                                                                        then pre.ctid
                                                                        else null 
                                                                        end pre_id,
                                                                        dpr.certif,
                                                                        sum(case 	
                                                                                ...
                                                                                then 	coalesce(dpr.capital,0) * case 
                                                                                                                                                        when addsuini = '1' 
                                                                                                                                                        then 1 
                                                                                                                                                        else 0 
                                                                                                                                                end
                                                                        ...
                                                                        end) premium, --prima contable
                                                                        SUM(case 
                                                                                when dpr.type_detai in ('3','4') and dpr.bill_item not in (4,5,9,97)
                                                                                then coalesce(dpr.premium,0)
                                                                                else 0 
                                                                        end) premium_recdes, --de la prima contable, los recargos y descuentos
                                                                        SUM(case 
                                                                                when dpr.type_detai in ('1','3','4') and dpr.bill_item not in (4,5,9,97)
                                                                                then coalesce(dpr.commision,0)
                                                                                else 0 
                                                                        end) commision --comisión
    ...
    L_RBRECPR_INSUNIX_LPG_RAMO_INCENDIO = '''
                                          (
                                                select 	'D' as INDDETREC,
                                                        'RBRECPR' as TABLAIFRS17,
                                                        '' PK,
    ...
    L_RBRECPR_VTIME_LPG_MULTIRIESGO_INCENDIO = '''
                                                 (
                                                        select	'D' as INDDETREC,
                                                                'RBRECPR' as TABLAIFRS17,
                                                                '' PK, 
    ...
    DF_LPG_VTIME_MULTIRIESGO_INCENDIO = glue_context.read.format('jdbc').options(**connection).option("fetchsize",10000).option("dbtable",L_RBRECPR_VTIME_LPG_MULTIRIESGO_INCENDIO).load()

    L_RBRECPR_VTIME_LPG_OTROS_RAMOS = '''
                                         (
                                        select	'D' as INDDETREC,
                                                'RBRECPR' as TABLAIFRS17,
                                                '' PK, 
    ...
    L_RBRECPR_INSIS_LPV = '''
                             (
                                ...
                                SELECT 	        'D' AS INDDETREC,
                                                ...
                                                '' VMTCOMR,
                                                COALESCE(
                                                                (SELECT (GRC."DISCOUNT" * A.NETA/100) 
                                                ...
                                                '' DDESTPPROC,
                                                '' DCASHBACK
                                FROM ( select	P."POLICY_ID" POLICY_ID,
                                                PEP."ENG_POL_TYPE" ENG_POL_TYPE,
                                                PEP."MASTER_POLICY_ID" MASTER_POLICY_ID,
                                                CASE 
                                                        WHEN BDO."DOC_TYPE_ID" = 19997 
                                                        THEN 'DEVOLUCIÓN'
                                                WHEN BDO."DOC_TYPE_ID" = 19713
                                                THEN 'COBRO'
                                                        END TIPO_PROFORMA,
                                                        ...
                                                        TR."CURRENCY" MONEDA,
                                                        CASE 
                                                                WHEN TR."PAID_STATUS" = 'Z' 
                                                        then SUM(CASE 
                                                                                WHEN TR."TRANSACTION_TYPE" = 'PREMIUM' 
                                                                                THEN TR."AMOUNT" 
                                                                        ELSE 0 
                                                                END)
                                                        else SUM(CASE 
                                                        ...
                                                        CASE
                                                                ...
                                                                END PRIMA_TOTAL,
                                                        CASE
                                                                WHEN TR."PAID_STATUS" IN ('N','P') THEN 'PENDIENTE'
                                                                WHEN TR."PAID_STATUS" = 'Y' THEN 'COBRADO'
                                                                WHEN TR."PAID_STATUS" = 'Z' THEN 'ANULADO'
                                                        END ESTADO_PROFORMA,
                                                        CASE
                                                                WHEN TR."PAID_STATUS" IN ('N','P') AND BDO."DOC_SUFFIX" IS NULL THEN CAST(BDO."DUE_DATE" AS DATE) --PROFORMA PENDIENTE 
                                                                WHEN TR."PAID_STATUS" IN ('N','P') AND BDO."DOC_SUFFIX" IS NOT NULL 
                                                                THEN --PROFORMA PENDIENTE
                                                        COALESCE(
                                                                (
                                                                SELECT  MAX(CAST(B."ACTION_DATE" AS DATE))            --- CORRECCIÓN EN LA FECHA DE DOC AUTORIZADO
                                                                ...
                                                                WHEN MIN(TR."PAID_STATUS") IN ('Z') 
                                                                THEN  --PROFORMA ANULADA
                                                        (
