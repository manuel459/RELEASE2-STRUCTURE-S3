def get_data(glue_context, connection, p_fecha_inicio, p_fecha_fin):
    ...
    INSUNIX_LPG_AMF  =  f'''
                            (select 
                             ...
                             cast(greatest(cla.compdate,cl0.xxxxdate) as varchar) TIOCFRM,
                             '' TIOCTO, --excluido
                             'PIG' KGIORIGM,
                             'LPG' KSCCOMPA,
                             ...
                             from (	select 	cla.ctid cla_id,
                                                                        ...
                                                                        (	select 	max(greatest(
                                                                                                        case when clm.compdate <= dat.inc_fin then clm.compdate else '0001-01-01'::date end,
                                                                                                        case when clh.compdate <= dat.inc_fin then clh.compdate else '0001-01-01'::date end,
                                                                                                        case when clh.operdate <= dat.inc_fin then clh.operdate else '0001-01-01'::date end,
                                                                                                        case when cla.compdate <= dat.inc_fin then cla.compdate else '0001-01-01'::date end))
                                                                                from	usinsug01.claim_his clh
                                                        ...
                                                        join	(	select	cast('{p_fecha_inicio}' as date) inc_ini, --rango inicial carga incremental (variable a elecci�n)
                                                                                cast('{p_fecha_fin}' as date) inc_fin --rango final carga incremental (variable a elecci�n)
                                                                        ) dat on 1 = 1
                                                        join	usinsug01.claim cla
                                                                ...
                                                                and		(cla.compdate between dat.inc_ini and dat.inc_fin --se consideran siniestros con modificaciones entre los rangos (limitaci�n carga inicial)
                                                                                or	exists --a nivel de transacciones
                                                                                        (   select  1
                                                                                                ...
                                                                                                and     (clh.operdate between dat.inc_ini and dat.inc_fin --se consideran siniestros con operaciones entre los rangos (limitaci�n carga inicial)
                                                                                                                or clh.compdate between dat.inc_ini and dat.inc_fin)) --se consideran siniestros con modificaciones entre los rangos (limitaci�n carga inicial)								
                                                                                        ...
                                                                                        (   select  1
                                                                                                from	usinsug01.claim_his clh
                                                                                                                ...
                                                                                                                and		clm.claim = clh.claim
                                                                                                                and		clm.movement = clh.transac
                                                                                                                and		clm.compdate between dat.inc_ini and dat.inc_fin --se consideran coberturas con modificaciones entre los rangos (limitaci�n carga inicial)
                                                                                                where	clh.claim = cla.claim
                                                                                                and		trim(clh.oper_type) in 
                                                                                                                (select cast(operation as varchar(2)) from usinsug01.tab_cl_ope where reserve = 1 or ajustes = 1 or pay_amount = 1)))
    ...
    INSUNIX_LPG_OTROS = f'''
                            ...
                            from (	select 	cla.ctid cla_id,
                                                                        ...
                                                                        coalesce((	select	sum(coalesce(
                                                                                                ...
                                                                                                where	clh.claim = cla.claim
                                                                                                and		trim(clh.oper_type) in (select cast(operation as varchar(2)) from usinsug01.tab_cl_ope where reserve = 1 or ajustes = 1)
                                                                                                and		clh.operdate <= dat.inc_fin),0) monto_trans, --evita tomar operaciones posteriores (carga incremental)
                                                                        coalesce((	select	evi.scod_vt
                                                                                                from	usinsug01.equi_vt_inx evi 
                                                                                                where	evi.scod_inx = cla.client),
                                                                                        cla.client) client,
                                                                        ...
                                                                        dat.inc_fin dat_lim
                                                        from 	usinsug01.policy pol
                                                        join	(	select	cast('01/01/2023' as date) inc_ini, --rango inicial carga incremental (variable a elecci�n)
                                                                                                cast('01/31/2023' as date) inc_fin --rango final carga incremental (variable a elecci�n)
                                                                        ) dat on 1 = 1
                                                        join	usinsug01.claim cla
                                                                on		cla.usercomp = pol.usercomp
                                                                and     cla.company = pol.company
                    ...
                    from 	(	select  cla.ctid cla_id,
                                            ...
                                            (   select  max(cpl."NCURRENCY")
                                                            ...
                                                            coalesce((	select	sum(case	coalesce(
                                                                                    ...
                                                                                    where	clh."NCLAIM" = cla."NCLAIM"
                                                                                    and		clh."NOPER_TYPE" in (select cast("SVALUE" as INT4) from usvtimg01."CONDITION_SERV" where "NCONDITION" in (71,72)) 
                                                                                    and     cast(clh."DOPERDATE" as date) <= dat.inc_fin),0) monto_trans,
                                                            case	pol."SBUSSITYP"
                                                                            when	'2'
                                                    ...
                                                    case	when	NOT (pol."SPOLITYPE" = '2' AND cla."NBRANCH" = 57 AND cla."NPRODUCT" = 1)
                                                                    ...
                                                                    ELSE    0 END flag_rea,
                                                            (	select 	max(greatest(
                                                                                            case when cast(clm."DCOMPDATE" as date) <= dat.inc_fin then cast(clm."DCOMPDATE" as date) else '0001-01-01'::date end,
                                                                                            case when cast(clh."DCOMPDATE" as date) <= dat.inc_fin then cast(clh."DCOMPDATE" as date) else '0001-01-01'::date end,
                                                                                            case when cast(clh."DOPERDATE" as date) <= dat.inc_fin then cast(clh."DOPERDATE" as date) else '0001-01-01'::date end,
                                                                                            case when cast(cla."DCOMPDATE" as date) <= dat.inc_fin then cast(cla."DCOMPDATE" as date) else '0001-01-01'::date end))
                                                                    from	usvtimg01."CLAIM_HIS" clh
                                                            ...
                                                            and		clm."NTRANSAC" = clh."NTRANSAC"
                                                                    ...
                                                                    and		(clh."DOPERDATE" <= dat.inc_fin or clh."DCOMPDATE" <= dat.inc_fin) --evita tomar operaciones posteriores (carga incremental)
                                                                    ) xxxxdate,
                                                            dat.inc_fin dat_lim
                                            from    (	select	ctid pol_id
                                            ...
                                            join	usvtimg01."CLAIM" cla
                                                    ...
                                                    and     cla."SSTACLAIM" <> '6'
                                                    and		(cast(cla."DCOMPDATE" as date) between dat.inc_ini and dat.inc_fin
                                                                            or	exists
                                                                    (   select  1
                                                                    from    usvtimg01."CLAIM_HIS" clh
                                                                    where	clh."NCLAIM" = cla."NCLAIM"
                                                                    and		coalesce(clh."NCASE_NUM",0) >= 0
                                                                                            and     (cast(clh."DOPERDATE" as date) between dat.inc_ini and dat.inc_fin --se consideran siniestros con operaciones entre los rangos (limitaci�n carga inicial)
                                                                                                            or cast(clh."DCOMPDATE" as date) between dat.inc_ini and dat.inc_fin) --se consideran siniestros con modificaciones entre los rangos (limitaci�n carga inicial)
                                                                    ...
                                                                    from    usvtimg01."CLAIM_HIS" clh
                                                                                    ...
                                                                                    and		clm."NDEMAN_TYPE" = clh."NDEMAN_TYPE"
                                                                                    and		clm."NTRANSAC" = clh."NTRANSAC"
                                                                                    and		cast(clm."DCOMPDATE" as date) between dat.inc_ini and dat.inc_fin --se consideran coberturas con modificaciones entre los rangos (limitaci�n carga inicial)
                                                                    where	clh."NCLAIM" = cla."NCLAIM"
                                                                    and		coalesce(clh."NCASE_NUM",0) >= 0
                                                                                            and		clh."NOPER_TYPE" in 
                    ...
                    (select	
                     ...
                     from 	(	select  cla.ctid cla_id,
                                             ...
                                             from    (	select	ctid pol_id
                                                                     ...
                                                                     where	"SCERTYPE" = '2') po0
                                             join	(	select	cast('01/01/2018' as date) inc_ini, --rango inicial carga incremental (variable a elecci�n)
                                                                                     cast('01/31/2018' as date) inc_fin --rango final carga incremental (variable a elecci�n)
                                                             ) dat on 1 = 1
    ...
    INSIS_LPV = f'''
                    (select	
                     ...
                     from	(	select	cla.cla_id,
                                                             cla.pol_id,
                                                             cla.pep_id,
                                                             max(cla.dat_lim) dat_lim, --valor �nico, se evita hacer group by por este campo
                                             ...
                                             from	(	select	cla."CLAIM_ID",
                                                                                     ...
                                                                                     (	select	max(greatest(
                                                                                                                     case when cast(cla."REGISTRATION_DATE" as date) <= dat.inc_fin then cast(cla."REGISTRATION_DATE" as date) else '0001-01-01'::date end,
                                                                                                                     case when cast(clh."REGISTRATION_DATE" as date) <= dat.inc_fin then cast(clh."REGISTRATION_DATE" as date) else '0001-01-01'::date end,
                                                                                                                     case when cast(clh."CHANGE_DATE" as date) <= dat.inc_fin then cast(clh."CHANGE_DATE" as date) else '0001-01-01'::date end))
                                                                     ...
                                                                     where	(cast(cla."REGISTRATION_DATE" as date) between dat.inc_ini and dat.inc_fin
                                                                                     or	exists 
                                                                                             (	select	1 
                                                                                                     from	usinsiv01."CLAIM_RESERVE_HISTORY" clh
                                                                                                     join	usinsiv01."CLAIM_OBJECTS" clo
                                                                                                     ...
                                                                                                     where	clh."CLAIM_ID" = cla."CLAIM_ID"
                                                                                                     and		clh."OP_TYPE" IN ('REG','EST','CLC','PAYMCONF','PAYMINV')
                                                                                                     and		(cast(clh."REGISTRATION_DATE" as date) between dat.inc_ini and dat.inc_fin --se consideran siniestros operaciones en el nuevo periodo
                                                                                                                     or cast(clh."CHANGE_DATE" as date) between dat.inc_ini and dat.inc_fin)))) cla --se consideran siniestros con modificaciones en el nuevo period
                                             join	usinsiv01."CLAIM_RESERVE_HISTORY" clh
                                                             on		clh."CLAIM_ID" = cla."CLAIM_ID"
                                                             and		clh."OP_TYPE" IN ('REG','EST','CLC','PAYMCONF','PAYMINV')
                                                             and		cast(clh."REGISTRATION_DATE" as date) <= cla.dat_lim --se consideran las operaciones hasta la fecha de corte (al ser cabecera, debe estar actualizado a la fecha de corte)
                                             join	usinsiv01."CLAIM_OBJECTS" clo
                                                             on		clo."CLAIM_ID" = clh."CLAIM_ID"
                                                             and		clo."CLAIM_OBJ_SEQ" = clh."CLAIM_OBJECT_SEQ"
                                                             and		clo."REQUEST_ID" = clh."REQUEST_ID"
