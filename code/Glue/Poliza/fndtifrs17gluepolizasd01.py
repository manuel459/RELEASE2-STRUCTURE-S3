...
try:
    ...
    #-------------------------------------#
    job_name = f'fndtifrs17gluepolizas{env}01'
    ...
    for config in l_dic_config['POLIZAS']['path_file_tmp']:
        if config['flag'] == 1:
            ...
            L_BUFFER_POLIZAS = io.BytesIO()
            L_DF_POLIZAS.toPandas().to_parquet(L_BUFFER_POLIZAS, index=False)
            L_BUFFER_POLIZAS.seek(0)
