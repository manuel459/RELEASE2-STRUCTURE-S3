def get_data(glue_context, connection):  
  l_abprcob_insunix_lpg = '''
                      (
                              ...
                              '''
  l_df_abprcob_insunix_lpg = glue_context.read.format('jdbc').options(**connection).option("dbtable",l_abprcob_insunix_lpg).load()
  ...
  l_df_abprcob_insunix_lpv = glue_context.read.format('jdbc').options(**connection).option("dbtable",l_abprcob_insunix_lpv).load()
  
  l_df_abprcob_insunix = l_df_abprcob_insunix_lpg.union(l_df_abprcob_insunix_lpv)
                                            ...
                                            AND	PR1."DNULLDATE" IS NULL) THEN NULL
                                                ELSE  CASE	
                                                  ...
                                                  ELSE NULL 
                                                          ...
                                                          END 
                                          END PRO_ID
                                    FROM USVTIMG01."PRODUCT" PRO) PR0, USVTIMG01."PRODUCT" PRO WHERE PRO.CTID = PR0.PRO_ID) P 
                          ON GC."NBRANCH" = P."NBRANCH" AND GC."NPRODUCT"  = P."NPRODUCT"
                        ) AS TMP
                        '''
                      
  l_df_abprcob_vtime_lpg = glue_context.read.format('jdbc').options(**connection).option("dbtable", l_abprcob_vtime_lpg).load()
  ...
  l_df_abprcob_vtime_lpv = glue_context.read.format('jdbc').options(**connection).option("dbtable", l_abprcob_vtime_lpv).load()
  
  
  l_df_abprcob_vtime = l_df_abprcob_vtime_lpg.union(l_df_abprcob_vtime_lpv)
  ...
  l_abprcob_insis = '''
                    (
                      ...
                      LEFT JOIN USINSIV01."CFG_NL_PRODUCT" C_NL_PROD ON C_NL_PROD."PRODUCT_LINK_ID" = CNC."PRODUCT_LINK_ID"
